(ns ncino.main
  "Kotodama WASM entrypoint for the Ncino clean-room actor (L4) — Clojure port.
  ...
  facts (`ncino.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "ncino_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                    :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "ncino_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"ncino.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ncino-compat" :tier tier :entities entities} 200])
